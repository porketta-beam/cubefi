                    ...
                    try:
                        with st.spinner("기존 VectorDB를 로드 중입니다..."):
                            ...
                            if db:
                                ...
                                st.success(f"기존 VectorDB를 성공적으로 로드했습니다! (문서 수: {db_status['count']})")
                                st.rerun()
            ...
            try:
                with st.spinner("문서를 처리 중입니다..."):
                    ...
                    if operation_mode == "기존 VectorDB에 문서 추가":
                        # 기존 VectorDB에 문서 추가
                        old_count = db_status["document_count"]
                    ...
                    if db:
                        ...
                        with col1:
                            if operation_mode == "기존 VectorDB에 문서 추가":
                                ...
                                updated_db_status = get_vectordb_status()
                                st.metric("전체 청크 수", updated_db_status["document_count"])
                            else:
...
st.sidebar.markdown("""
### 사용 방법:
1. **문서 임베딩 탭**: 개별 파일을 업로드하고 처리
2. **Raw Data 동기화 탭**: raw_data 폴더와 자동 동기화
3. **RAG 테스트 탭**: RAG 시스템과 실시간 채팅
4. **질문 생성하기 탭**: 문서 기반 질문 자동 생성
5. **RAG 평가하기 탭**: RAGAS로 정량적 평가
